...
@section('section')
    <div class="p-2 container-fluid" style="padding-inline: 4.5rem !important;">
        ...
        <p>With reference to your application, to offer you admission into the BSPH to pursue a ({{ $filters->where('program', $_program->id)->first()['duration']??'#' }}) {{ \App\Helpers\Helpers::instance()->numToWord($filters->where('program', $_program->id)->first()['duration']??0) }}-year <b style="text-transform: capitalize;"> {{ $_degree->deg_name }} degree</b> in <b>{{ $department }}</b></p>
